...
void xhci_post_command(xhci_t* xhci, uint32_t command, uint64_t ptr, uint32_t control_bits,
    ...
    mutex_acquire(&xhci->command_ring.mutex);
    ...
    mutex_release(&xhci->command_ring.mutex);
}
...
static void xhci_handle_mfindex_wrap(xhci_t* xhci) {
    mutex_acquire(&xhci->mfindex_mutex);
    xhci->mfindex_wrap_count++;
    xhci->last_mfindex_wrap = current_time();
    mutex_release(&xhci->mfindex_mutex);
}
...
uint64_t xhci_get_current_frame(xhci_t* xhci) {
    ...
    if (mfindex < 500) {
        if (current_time() - xhci->last_mfindex_wrap > 1000) {
