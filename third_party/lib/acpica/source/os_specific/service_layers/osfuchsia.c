...
static pthread_mutex_t os_execute_lock = PTHREAD_MUTEX_INITIALIZER;
static mx_handle_t os_execute_event;
...
ACPI_STATUS AcpiOsInitialize() {
    os_execute_event = mx_event_create(0);
    if (os_execute_event < 0) {
        return AE_ERROR;
    }
...
void *AcpiOsMapMemory(
    ...
    mx_status_t status = mx_acpi_mmap(aligned_address, end - aligned_address, &vaddr);
    if (status != NO_ERROR) {
        return NULL;
    }
...
static void* acpi_os_task(void *raw_ctx) {
    struct acpi_os_task_ctx *ctx = raw_ctx;

    ctx->func(ctx->ctx);

    pthread_mutex_lock(&os_execute_lock);
    os_execute_tasks--;
    if (os_execute_tasks == 0) {
        mx_object_signal(os_execute_event, MX_SIGNAL_SIGNALED, 0);
    }
    pthread_mutex_unlock(&os_execute_lock);
...
ACPI_STATUS AcpiOsExecute(
    ...
    pthread_mutex_lock(&os_execute_lock);
    os_execute_tasks++;
    mx_object_signal(os_execute_event, 0, MX_SIGNAL_SIGNALED);
    pthread_mutex_unlock(&os_execute_lock);

    pthread_t thread;
    int ret = pthread_create(&thread, NULL, acpi_os_task, ctx);
    if (ret != 0) {
        free(ctx);
        pthread_mutex_lock(&os_execute_lock);
        os_execute_tasks--;
...
void AcpiOsWaitEventsComplete(void) {
    while (1) {
        ...
        mx_status_t status = mx_handle_wait_one(os_execute_event,
                                                MX_SIGNAL_SIGNALED,
                                                MX_TIME_INFINITE,
                                                &state);
        if (status != NO_ERROR) {
...
static void* acpi_irq_thread(void *arg) {
...
ACPI_STATUS AcpiOsInstallInterruptHandler(
    ...
    pthread_t thread;
    int ret = pthread_create(&thread, NULL, acpi_irq_thread, arg);
    if (ret != 0) {
        free(arg);
        return AE_ERROR;
    }
