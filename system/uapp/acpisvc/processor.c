...
static mx_status_t send_error(mx_handle_t h, mx_status_t status);
...
static mx_status_t dispatch(mx_handle_t h, void* _ctx, void* cookie) {
    ...
    } else if (status != ERR_NOT_ENOUGH_BUFFER || num_handles > 0) {
    ...
    if (num_bytes < sizeof(struct acpi_cmd_hdr)) {
        status = ERR_INVALID_ARGS;
        goto cleanup;
    }

    struct acpi_cmd_hdr* hdr = buf;
    if (hdr->version != 0) {
        status = send_error(h, ERR_NOT_SUPPORTED);
        goto cleanup;
    }
    if (hdr->len != num_bytes) {
        status = send_error(h, ERR_INVALID_ARGS);
        goto cleanup;
    }
...
static mx_status_t cmd_list_children(mx_handle_t h, acpi_handle_ctx_t* ctx, void* _cmd) {
    ...
    struct acpi_cmd_list_children* cmd = _cmd;
    if (cmd->hdr.len != sizeof(*cmd)) {
        return send_error(h, ERR_INVALID_ARGS);
    }
    ...
    while (num_children < rsp->num_children) {
        ...
        if (acpi_status == AE_NO_MEMORY) {
            status = send_error(h, ERR_NO_MEMORY);
            goto cleanup;
        } else if (acpi_status != AE_OK) {
            status = ERR_BAD_STATE;
            goto cleanup;
...
static mx_status_t cmd_get_child_handle(mx_handle_t h, acpi_handle_ctx_t* ctx, void* _cmd) {
    ...
    struct acpi_cmd_get_child_handle *cmd = _cmd;
    ...
    if (acpi_status != AE_OK) {
        return send_error(h, ERR_NOT_FOUND);
    }
    ...
    if (status != NO_ERROR) {
        free(child_ctx);
        return send_error(h, status);
    }

    status = mxio_dispatcher_add(dispatcher, msg_pipe[1], child_ctx, NULL);
    if (status != NO_ERROR) {
        status = send_error(h, status);
        goto cleanup;
    }

    struct acpi_rsp_get_child_handle rsp = {
        .hdr = {
            .status = NO_ERROR,
            .len = sizeof(rsp),
        },
...
static mx_status_t cmd_get_pci_init_arg(mx_handle_t h, acpi_handle_ctx_t* ctx, void* _cmd) {
    ...
    struct acpi_cmd_get_pci_init_arg *cmd = _cmd;
    if (cmd->hdr.len != sizeof(*cmd)) {
        return send_error(h, ERR_INVALID_ARGS);
    }
    ...
    if (!(info->Flags & ACPI_PCI_ROOT_BRIDGE)) {
        ACPI_FREE(info);
        return send_error(h, ERR_WRONG_TYPE);
    }
    ACPI_FREE(info);

    struct acpi_rsp_get_pci_init_arg* rsp = NULL;
    ...
    uint32_t len = arg_size + offsetof(struct acpi_rsp_get_pci_init_arg, arg);
    rsp = malloc(len);
    if (!rsp) {
        status = send_error(h, ERR_NO_MEMORY);
        goto cleanup;
