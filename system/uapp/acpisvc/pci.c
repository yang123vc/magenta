...
static ACPI_STATUS handle_prt(
    ...
    for (entry = (ACPI_PCI_ROUTING_TABLE *)entry_addr;
        ...
        uint32_t global_irq = PCI_NO_IRQ_MAPPING;
        ...
        if (entry->Source[0]) {
            ...
            while (res->Type != ACPI_RESOURCE_TYPE_END_TAG) {
                if (res->Type == ACPI_RESOURCE_TYPE_EXTENDED_IRQ) {
                    ...
                    if (global_irq != PCI_NO_IRQ_MAPPING) {
...
static mx_status_t find_pcie_config(mx_pci_init_arg_t* arg)
{
    ...
    arg->ecam_windows[0].base = table_start->Address + size_per_bus *
            arg->ecam_windows[0].bus_start;
